...
export function useTestDataService() {
    ...
    async function fetchAllTestDataTypes(payload) {
        ...
        await axios.get(`${BASE_URL_API}/v1/queue/omni-types`, payload).then(res => {
            setQueues(res.data)
    ...
    async function getTestDataType(testDataType) {
        await axios.get(`${BASE_URL_API}/v1/queue/omni/${testDataType}`).then(res => {
    ...
    async function createDataQueue(payload) {
        await axios.post(`${BASE_URL_API}/v1/queue/omni-type`, payload).then(res => {
            return res.data
        }).catch(function (error) {
            console.log(error)
        });
    ...
    async function updateDataQueue(id, payload) {
        await axios.put(`${BASE_URL_API}/v1/queue/omni-type/${id}`, payload).then(res => {
    ...
    async function deleteQueue(id) {
        await axios.delete(`${BASE_URL_API}/v1/queue/omni-type/${id}`).then(res => {
